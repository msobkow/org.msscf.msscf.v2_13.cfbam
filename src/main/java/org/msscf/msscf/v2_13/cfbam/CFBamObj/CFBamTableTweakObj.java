// Description: Java 11 base object instance implementation for CFBam TableTweak.
	...
	implements ICFBamTableTweakObj
{
	...
	public ICFBamTweakObj realise() {
		ICFBamTableTweakObj retobj = ((ICFBamSchemaObj)schema).getTableTweakTableObj().realiseTableTweak(
			(ICFBamTableTweakObj)this );
		return( (ICFBamTweakObj)retobj );
	}

	public void forget() {
		((ICFBamSchemaObj)schema).getTableTweakTableObj().reallyDeepDisposeTableTweak( (ICFBamTableTweakObj)this );
	}

	public ICFBamTweakObj read() {
		ICFBamTableTweakObj retobj = ((ICFBamSchemaObj)schema).getTableTweakTableObj().readTableTweakByIdIdx( getPKey().getRequiredTenantId(),
	...
	public ICFBamTableTweakTableObj getTableTweakTable() {
		return( ((ICFBamSchemaObj)schema).getTableTweakTableObj() );
	}

	public CFBamTweakBuff getBuff() {
		if( buff == null ) {
			if( isNew ) {
				buff = ((ICFBamSchema)schema.getBackingStore()).getFactoryTableTweak().newBuff();
			}
			else {
				// Read the data buff via the backing store
				buff = ((ICFBamSchema)schema.getBackingStore()).getTableTableTweak().readDerivedByIdIdx( ((ICFBamSchemaObj)schema).getAuthorization(),
	...
	public ICFBamTweakEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)schema).getTableTweakTableObj().lockTableTweak( getPKey() );
		}
		edit = ((ICFBamSchemaObj)schema).getTableTweakTableObj().newEditInstance( lockobj );
	...
	public ICFBamTableObj getRequiredContainerTableDef( boolean forceRead ) {
		if( ( requiredContainerTableDef == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredContainerTableDef = ((ICFBamSchemaObj)schema).getTableTableObj().readTableByIdIdx( getPKey().getRequiredTenantId(),
					getTableTweakBuff().getRequiredTableId(), forceRead );
			}
		}
