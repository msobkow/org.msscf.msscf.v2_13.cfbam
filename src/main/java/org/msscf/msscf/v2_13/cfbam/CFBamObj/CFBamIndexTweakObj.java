// Description: Java 11 base object instance implementation for CFBam IndexTweak.
	...
	implements ICFBamIndexTweakObj
{
	...
	public ICFBamTweakObj realise() {
		ICFBamIndexTweakObj retobj = ((ICFBamSchemaObj)schema).getIndexTweakTableObj().realiseIndexTweak(
			(ICFBamIndexTweakObj)this );
		return( (ICFBamTweakObj)retobj );
	}

	public void forget() {
		((ICFBamSchemaObj)schema).getIndexTweakTableObj().reallyDeepDisposeIndexTweak( (ICFBamIndexTweakObj)this );
	}

	public ICFBamTweakObj read() {
		ICFBamIndexTweakObj retobj = ((ICFBamSchemaObj)schema).getIndexTweakTableObj().readIndexTweakByIdIdx( getPKey().getRequiredTenantId(),
	...
	public ICFBamIndexTweakTableObj getIndexTweakTable() {
		return( ((ICFBamSchemaObj)schema).getIndexTweakTableObj() );
	}

	public CFBamTweakBuff getBuff() {
		if( buff == null ) {
			if( isNew ) {
				buff = ((ICFBamSchema)schema.getBackingStore()).getFactoryIndexTweak().newBuff();
			}
			else {
				// Read the data buff via the backing store
				buff = ((ICFBamSchema)schema.getBackingStore()).getTableIndexTweak().readDerivedByIdIdx( ((ICFBamSchemaObj)schema).getAuthorization(),
	...
	public void setBuff( CFBamTweakBuff value ) {
		if( ! ( ( value == null ) || ( value instanceof CFBamIndexTweakBuff ) ) ) {
	...
	public ICFBamTweakEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)schema).getIndexTweakTableObj().lockIndexTweak( getPKey() );
		}
		edit = ((ICFBamSchemaObj)schema).getIndexTweakTableObj().newEditInstance( lockobj );
	...
	public ICFBamIndexObj getRequiredContainerIndexDef( boolean forceRead ) {
		if( ( requiredContainerIndexDef == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredContainerIndexDef = ((ICFBamSchemaObj)schema).getIndexTableObj().readIndexByIdIdx( getPKey().getRequiredTenantId(),
					getIndexTweakBuff().getRequiredIndexId(), forceRead );
			}
		}
